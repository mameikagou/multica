...
func TestClassifyCursorConnectTimeout(t *testing.T) {
	...
	const writeFailure = "cursor-agent prompt write failed: write |1: broken pipe (result_seen=false, exit_code=1, scanner_error=false, event_count=0, invalid_event_count=0, last_event_type=none); actions completed before finalization may already have taken effect; cursor stderr: " + stderr
	for _, input := range []string{stderr, wrapped, strings.ToUpper(wrapped), writeFailure, strings.ToUpper(writeFailure)} {
...
func TestClassifyCursorConnectTimeoutScope(t *testing.T) {
	...
	for _, input := range []string{
		...
		"cursor-agent prompt write failed: write |1: broken pipe",
		"cursor-agent prompt write failed: write |1: broken pipe; cursor stderr: local tool failed: connect ETIMEDOUT 192.0.2.1:443",
		"cursor-agent prompt write failed: write |1: broken pipe; cursor stderr: MCP server error: Error: [unavailable] connect ETIMEDOUT 192.0.2.1:443",
		"cursor-agent prompt write failed: write |1: broken pipe; cursor stderr: Error: [unavailable] connect ETIMEDOUT_OTHER 192.0.2.1:443",
		"some tool prompt write failed: broken pipe; cursor stderr: Error: [unavailable] connect ETIMEDOUT 192.0.2.1:443",
	} {
		if got := Classify(input); got == ReasonAgentProviderNetwork {
