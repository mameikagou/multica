...
type Daemon struct {
	...
	// Publish every held environment claim with its exact release signal.
	// Follow-up turns can subscribe atomically instead of missing the handoff.
...
func gateResumeToReachableSession(task *Task, taskCtx *execenv.TaskContextForEnv, provider, envWorkDir string, sessionHomeReachable, refusesMissingSessionCwd bool, taskLog *slog.Logger) bool {
	...
	if providerUsesPiSessionFile(provider) {
		...
		// Codex addresses a thread by ID in CODEX_HOME, not by cwd. The
		// rollout-presence gate below is the authoritative resume proof.
		// Preserve the old fork's cwd-independent resume when a chat moves
		// from native cwd to managed execution or needs a fresh task root.
...
func (d *Daemon) lockEnvRootForReuseWaitingOutTheBusyWindow(
	...
	waited := false
	for {
		claim, info, released, err := d.lockEnvRootForReuse(wsRoot, rel, priorRoot)
		...
		case <-released:
			// The release notification is only a wakeup. Retake the OS lock
			// and revalidate ownership before reusing the environment.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	if env == nil {
		...
		// Reuse may decline even after its prior root was validated and locked.
		// Keep the old fork's single-rollout handoff, but only from the exact
		// managed directory whose identity is still protected by that claim.
		if provider == "codex" && reusable && lockedPriorInfo != nil {
			verifiedDir, owned := shouldReusePriorWorkdir(task, localAssignment, d.cfg.WorkspacesRoot)
			currentInfo, statErr := os.Stat(filepath.Dir(priorWorkDir))
			if owned && verifiedDir == priorWorkDir && statErr == nil && os.SameFile(lockedPriorInfo, currentInfo) {
				prepParams.CodexResumeSessionsSource = filepath.Join(filepath.Dir(priorWorkDir), "codex-home", "sessions")
			}
		}
	...
	// A reused workdir is not required for a Codex resume: the
	...
	prompt, resumedPrompt := buildTaskExecutionPrompts(task, provider, promptOptions...)
